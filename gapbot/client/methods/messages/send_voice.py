...
class SendVoice(BaseClient):
    def send_voice(self, chat_id, path, reply_keyboard=None, inline_keyboard=None, reply_form=None):
        return self._send(
            method='sendMessage',
            data={
                'chat_id': chat_id,
                'type': 'voice',
                'data': dumps(self.upload(chat_id=chat_id, file_type='voice', path=path).json()),
