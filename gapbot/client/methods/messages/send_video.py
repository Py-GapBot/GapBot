...
class SendVideo(BaseClient):
    def send_video(self, chat_id, path, reply_keyboard=None, inline_keyboard=None, reply_form=None):
        return self._send(
            method='sendMessage',
            data={
                'chat_id': chat_id,
                'type': 'video',
                'data': dumps(self.upload(chat_id=chat_id, file_type='video', path=path).json()),
