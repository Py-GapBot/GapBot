...
class SendFile(BaseClient):
    def send_file(self, chat_id, path, reply_keyboard=None, inline_keyboard=None, reply_form=None):
        return self._send(
            method='sendMessage',
            data={
                'chat_id': chat_id,
                'type': 'file',
                'data': dumps(self.upload(chat_id=chat_id, file_type='file', path=path).json()),
